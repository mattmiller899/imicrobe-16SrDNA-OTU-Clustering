...
def test_create_output_dir():
    with tempfile.TemporaryDirectory() as work_dir:
        output_dir = pipeline.create_output_dir(output_dir_name='test', parent_dir=work_dir)
...
def test_get_forward_fastq_files():
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        forward_fastq_fps = sorted(pipeline.get_forward_fastq_files(test_dir))
...
def test_get_forward_fastq_files__exception():
    with tempfile.TemporaryDirectory() as test_dir:
        ...
        with pytest.raises(pipeline.PipelineException):
            pipeline.get_forward_fastq_files(test_dir)
